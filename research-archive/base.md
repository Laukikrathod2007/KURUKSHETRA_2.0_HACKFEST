# Kurukshetra — Complete Technical Feature Base & Real-World Use Cases
## All 36 Features Explained with Exact Banking Protocols, Detection Logic, and Zero-Surveillance Privacy Boundaries
...
```
┌─────────────────┐       ┌────────────────────────┐       ┌─────────────────┐
│ Client UPI App  │ ───>  │ Kurukshetra Middleware │ ───>  │ NPCI UPI Switch │
│ (PhonePe/GPay)  │ <───  │ (MCP Intelligence API) │ <───  │ & Bank Core CBS │
└─────────────────┘       └────────────────────────┘       └─────────────────┘
```
...
* **Real-World Use Case (Fake OLX Buyer Scam):**
  * *Scenario:* Rajesh posts an antique sofa on OLX for ₹18,000. A buyer claiming to be an army officer messages: *"I want to buy this immediately. Verify my defense canteen clearance UPI ID first: `military.canteen.cctv@oksbi`."*
  * *Detection:* Over the preceding 48 hours, 42 prospective sellers across India typed this VPA to verify it, but 39 immediately aborted without paying upon noticing the resolved bank name was "Ramesh G — Savings Account". The switch logs an abandonment ratio of 93%.
  * *Interception:* When Rajesh inputs the VPA, Kurukshetra warns: *"High Abandonment Rate: 39 out of 42 people who checked this account recently chose not to send money."* Rajesh cancels the transfer.
...
* **Real-World Use Case (Telegram Task Scam Outbreak):**
  * *Scenario:* At 2:00 PM, a cyber fraud syndicate blasts 5,000 users on Telegram offering a part-time job rating hotels. Victims are told to send a ₹2,000 "registration deposit" to `vip.merchant.desk@paytm`.
  * *Detection:* Between 2:00 PM and 2:30 PM, 54 distinct UPI users across 4 different apps resolve `vip.merchant.desk@paytm`. The baseline lookup velocity for this 4-day-old account was 0.02 lookups/hour; it has surged to 108 lookups/hour.
  * *Interception:* Kurukshetra flags an active campaign burst. Every subsequent lookup shows: *"Active High-Volume Alert: 50+ users are resolving this account right now. This velocity signature matches mass job/investment scams."*
...
* **Real-World Use Case (Digital Arrest / CBI Impersonation):**
  * *Scenario:* Mrs. Anita is threatened by an impersonator claiming to be a CBI Inspector investigating an illegal courier. The caller tells her to send ₹75,000 "provisional court security bail" to `cbi.clearance.cell@sbi`.
  * *Detection:* The handle contains the keyword `cbi`. However, NPCI `RespValAdd` returns: Registered Name: *"Manoj Kumar"*, Account Category: *"Individual Savings"*, MCC: `0000`. In Indian law, government courts and investigative agencies never collect bail into personal savings accounts.
  * *Interception:* Hard contradiction screen: *"CRITICAL MISMATCH: You are transferring funds to an individual personal savings account belonging to Manoj Kumar. Official law enforcement agencies NEVER collect security bail via personal savings accounts."*
...
* **Real-World Use Case (Electricity Disconnection Scam):**
  * *Scenario:* A homeowner receives an urgent SMS: *"Power will be cut at 9:30 PM tonight due to unpaid electricity bill of ₹1,450. Pay immediately to `tneb.billing.officer@oksbi`."*
  * *Detection:* The handle contains `tneb` (Tamil Nadu Electricity Board) and `officer`. Bank records confirm the account is a private savings account opened 12 days ago with basic OTP-based KYC.
  * *Interception:* The payment is blocked: *"Impersonation Handle Detected: This UPI ID uses utility authority keywords (`tneb`), but belongs to a private individual savings account."*
...
* **Real-World Use Case (Mule Recruitment Ring):**
  * *Scenario:* An attacker running an illegal instant-loan extortion racket gives a borrower a phone number to pay ₹8,000.
  * *Detection:* Central Mapper query reveals that this single mobile number is linked to 9 different UPI IDs across 5 different regional banks, with 7 of those handles registered within the past 14 days.
  * *Interception:* The system flags the recipient as a high-density mule aggregator node: *"High Risk: This phone number is associated with 9 newly created UPI handles across multiple banks — a signature of mule collection networks."*
...
* **Real-World Use Case (Coerced High-Ticket Transfer):**
  * *Scenario:* A retired teacher who normally uses UPI only for ₹150–₹500 grocery payments is pressured by a scammer into sending ₹55,000 for a "customs clearance penalty".
  * *Detection:* ₹55,000 is 110x the user's median first-time transaction value ($\text{Z-Score} > 4.5$) to a recipient with zero transaction history.
  * *Interception:* The app pauses before the PIN screen to present an educational verification step rather than allowing a 1-second impulsive transfer.
...
* **Real-World Use Case (Instant Cash-Out Mule):**
  * *Scenario:* A victim transfers ₹40,000 to an account provided by an online fraudster.
  * *Detection:* The recipient account shows that over the past 7 days, 98.2% of all inbound funds were transferred out via IMPS or withdrawn at ATMs within 4 minutes of receipt. The account maintains a near-zero average balance.
  * *Interception:* The recipient is identified as a pass-through transit mule. Kurukshetra enforces a 4-hour cooling-off hold, preventing immediate cash draining.
...
* **Real-World Use Case (Remote Extortion Target):**
  * *Scenario:* A resident in Kochi is targeted by a cybercriminal based in Mewat, Rajasthan, asking for ₹20,000.
  * *Detection:* The Kochi sender and the Mewat recipient share zero 1st, 2nd, or 3rd-degree graph connections. The recipient has no transactional overlap with any node in South India.
  * *Interception:* Combined with low account age, graph isolation elevates the composite risk score, triggering an intervention challenge.
...
* **Real-World Use Case (Marketplace "Scan-to-Receive" Refund Fraud):**
  * *Scenario:* A seller on Facebook Marketplace is told by an interested buyer: *"I have sent you a QR code on WhatsApp for ₹12,000 advance. Just scan it in your GPay to receive the payment into your account."*
  * *Detection:* The user opens the QR image. The URI contains `upi://pay?pa=mule7@oksbi&am=12000`. The recipient is a personal savings account.
  * *Interception:* The app displays an unmissable warning: *"CRITICAL REMINDER: Scanning a QR code ALWAYS SENDS money. You never need to scan a QR code or enter your UPI PIN to receive money."* The seller realizes the trap and cancels.
...
* **Real-World Use Case (Bank NetBanking Phishing SMS):**
  * *Scenario:* A customer receives an SMS: *"Your SBI YONO account will be blocked today. Click here to verify: `upi://pay?pa=sbi.kyc.portal@axis&am=15000&tn=KYC_REACTIVATION`."*
  * *Detection:* The link embeds a ₹15,000 debit instruction, has a fraudulent note `KYC_REACTIVATION`, and points to an Axis Bank personal VPA claiming to be SBI.
  * *Interception:* The deep link is intercepted before the payment screen is rendered, displaying an alert that the link is an SMS phishing attack.
...
* **Real-World Use Case (Predatory Loan App Repayment):**
  * *Scenario:* A victim of an illegal instant-loan app is harassed to pay ₹10,000 to `recovery.desk@icici`.
  * *Detection:* Over the previous 72 hours, 8 other borrowers who transferred funds to this VPA submitted post-transaction fraud reports citing blackmail and morphed photo threats.
  * *Interception:* When the new victim initiates payment, the app displays: *"Community Fraud Alert: 8 users reported this account for illegal loan harassment in the past 3 days."*
...
* **Real-World Use Case (Coordinated Extortion Ring):**
  * *Scenario:* A scammer calling from `+91-98711-XXXXX` convinces Victim A in Mumbai to pay ₹40,000 to `mule88@kotak`. Two hours later, the same number calls Victim B in Bengaluru and instructs her to pay ₹35,000 to `mule88@kotak`.
  * *Detection:* The central switch correlates that both victims received calls from `+91-98711-XXXXX` shortly before initiating transfers to the exact same recipient account.
  * *Interception:* Kurukshetra establishes a high-confidence criminal syndicate link. Victim B's payment is hard-blocked, and the VPA is blacklisted across the entire UPI ecosystem.
...
* **Real-World Use Case (Viral Social Media Task Campaign):**
  * *Scenario:* A fraudulent investment scheme goes viral online, promising 300% returns if users transfer ₹3,000 to `growfast@sbi`.
  * *Detection:* Across India, 320 users enter the UPI ID to check the name. 308 of them see an unknown personal name and abandon the flow. The switch observes a 25:1 fan-in ratio within 90 minutes.
  * *Interception:* Kurukshetra flags the VPA as an active campaign target before the initial 12 victims even realize they have been defrauded or file bank complaints, protecting hundreds of subsequent users.
...
* **Real-World Use Case (Crypto Investment / Pig Butchering Scam):**
  * *Scenario:* A victim is lured into a fake crypto platform:
  ...
  * *Detection:* The transaction sequence ($₹1,000 \to ₹5,000 \to ₹25,000 \to ₹1,20,000$) exhibits geometric scaling ($k \approx 5$) with shrinking intervals.
  * *Interception:* The payment is halted: *"Pattern Match: Pig-Butchering Investment Scam. Fraudsters allow small withdrawals early to build confidence before demanding large non-refundable sums."*
...
* **Real-World Use Case (Coerced Blackmail Splitting):**
  * *Scenario:* An extortionist tells a victim: *"Send ₹50,000 immediately, but do not send it in one shot or your bank will stop it. Send ₹9,999 five times right now."*
  * *Detection:* The user sends ₹9,999 at 10:02 AM, ₹9,999 at 10:07 AM, ₹9,999 at 10:13 AM, and attempts a 4th at 10:19 AM.
  * *Interception:* Kurukshetra identifies structured threshold evasion (cumulative ₹39,996 across 4 transfers, each just below ₹10,000) and halts the sequence to prevent account draining.
...
* **Real-World Use Case ("Accidental Transfer" Trap):**
  * *Scenario:* A user receives ₹10 in their account with note "UPI". Minutes later, a man calls weeping: *"Sir, I am a poor farmer. I accidentally transferred ₹50,000 to your UPI instead of the hospital! Please check your bank SMS and return my ₹50,000!"* The victim sees a recent bank SMS, gets confused, and prepares to transfer ₹50,000.
  * *Detection:* Ledger lookup reveals the actual inbound deposit from this entity was ₹10, while the outbound attempt is ₹50,000 (ratio 5,000:1).
  * *Interception:* Kurukshetra displays: *"Accidental Transfer Trap Detected: You received only ₹10 from this person, NOT ₹50,000. Do not send any money back."*
...
* **Real-World Use Case (OLX Buyer Collect Request Scam):**
  * *Scenario:* A seller is told by an online buyer: *"I have sent you a ₹5,000 deposit via UPI. Just accept the collect request and enter your UPI PIN to receive the money into your account."*
  * *Detection:* The incoming notification is a *debit* collect request for ₹5,000 with note *"Cashback Received - Tap to Claim"*.
  * *Interception:* The confirmation screen displays a red alert: *"DANGER: Entering your UPI PIN will DEDUCT ₹5,000 from your account. A UPI PIN is NEVER required to receive money."*
...
* **Real-World Use Case (Money Mule Layering Ring):**
  * *Scenario:* A mule account receives 35 deposits totaling ₹5.8 Lakhs in 4 days from victims across India, with zero payments for food, fuel, or utilities.
  * *Detection:* The account exhibits a 99.1% sink ratio, with all outbound funds routed to cryptocurrency OTC desks.
  * *Interception:* When a new victim attempts payment, Kurukshetra identifies the account as an active mule sink and blocks the transfer.
...
* **Real-World Use Case (Compromised Dormant Account):**
  * *Scenario:* A criminal ring buys access to a dormant student account with an average balance of ₹150 over the past year. In 48 hours, it receives 16 transfers totaling ₹3.2 Lakhs, all withdrawn at ATMs within 5 minutes of arrival.
  * *Detection:* The temporal profile matches the Burst-Drain-Dormant signature.
  * *Interception:* Transfers to this account are paused with a 4-hour cooling-off hold, preventing immediate cash draining.
...
* **Real-World Use Case (Industrial Boiler Room Operation):**
  * *Scenario:* A commercial call center running fake courier scams operates strictly from 10:00 AM to 5:30 PM on weekdays.
  * *Detection:* 100% of all credits into the recipient account arrive between 10:00 AM and 5:30 PM on weekdays, with zero weekend or night activity.
  * *Interception:* Contributes to the composite risk score, distinguishing commercial boiler rooms from authentic personal accounts.
...
* **Real-World Use Case (Cross-State Mule Syndicate):**
  * *Scenario:* A victim in Pune is directed to pay ₹35,000 to an account in Assam.
  * *Detection:* The Assam account was opened 5 days ago with basic OTP KYC. Over the last 48 hours, it received payments from 16 distinct users across 7 different states, with funds cleared within 4 minutes.
  * *Interception:* High geographic dispersion + low account age + instant velocity triggers a composite risk score of 0.96. The payment is hard-blocked.
...
* **Real-World Use Case (Synthetic Identity / Student Mule):**
  * *Scenario:* An account belonging to an 18-year-old whose PAN lists "Student" with annual income < ₹1 Lakh is suddenly receiving ₹4 Lakhs daily.
  * *Detection:* Declared PAN profile clashes with transactional volume, and an allied bank account tied to the same Aadhaar was recently frozen by state police.
  * *Interception:* Immediate block: *"High-Risk Account: Recipient identity credentials have active regulatory freeze flags."*
...
* **Real-World Use Case (Active Police FIR Match):**
  * *Scenario:* A senior citizen in Jaipur is targeted by a stock advisory scam. Two hours prior, a victim in Chandigarh reported the same UPI ID to the 1930 Cybercrime Helpline.
  * *Detection:* I4C registers the VPA in the national fraud database. When the Jaipur victim initiates transfer, Kurukshetra's query returns an active match.
  * *Interception:* The payment is blocked before settlement: *"MHA Cybercrime Registry Alert: This account is currently frozen under active cybercrime investigation (CFCFRMS Incident #2026/IN-8891)."*
...
* **Real-World Use Case (Spoofed Telecom Caller):**
  * *Scenario:* A scammer calling from a mobile number claims to be an officer from Mumbai Customs.
  * *Detection:* Sanchar Saathi lookup reveals the SIM card was reported lost/fraudulently issued 3 days ago and has 35+ spam complaints on TRAI DND.
  * *Interception:* System alerts: *"Caller Identity Alert: The calling number is registered on national spam registries and flagged in Sanchar Saathi."*

---

# Category H: Intelligent Intervention Design (7 Features)
...
* **Technical Mechanism (How It Works):** 
  Instead of displaying an ambiguous warning dialog, the app renders a step-by-step visual narrative stepper illustrating the exact script of the scam. Shows the victim where they currently are and what the scammer will demand next.
* **Real-World Use Case (Digital Arrest Playbook):**
  * *Scenario:* A victim under pressure from a fake CBI officer prepares to pay ₹60,000.
  * *Interception Display:*
    * *Step 1:* Caller claims illegal parcel / Aadhaar crime.
    * *Step 2:* Threat of non-bailable arrest warrant.
    * *Step 3:* Demands "temporary security deposit" to personal account.
    * 👉 **[YOU ARE HERE - STEP 3]**
    * *Step 4:* After paying, they will claim clearance failed and demand ₹1.5 Lakhs more. They will never stop.
  * *Outcome:* Seeing the exact script dismantles the scammer's authority. The victim terminates the call.
...
* **Real-World Use Case (Elder Financial Extortion):**
  * *Scenario:* A 72-year-old grandfather is told his grandson is arrested after a college altercation and needs ₹40,000 for hospital release immediately.
  * *Detection & Intervention:* High-risk new beneficiary transfer. The app initiates a 15-minute cooling hold and pushes an alert to his daughter's phone: *"Your father is attempting an urgent ₹40,000 transfer under potential scam coercion. Call him immediately."*
  * *Outcome:* The daughter calls her father, confirms the grandson is sitting in class, and stops the transfer.
...
* **Real-World Use Case (Court Bail / Government Fine Contradiction):**
  * *Scenario:* A user intimidated by a scammer attempts to pay a "court penalty".
  * *Interception:* App asks: *"What is this payment for?"* The user selects *"Government Fine / Court Bail"*.
  * *Contradiction Trigger:* Recipient is `arun.kumar1988@sbi` (Individual Savings, `mc=0000`).
...
* **Technical Mechanism (How It Works):** 
  Requires the user to record a 10-second audio clip stating whom they are paying and why. Forces verbalization of the situation, engaging critical cognitive faculties and disrupting the emotional trance created by the scammer.
* **Real-World Use Case (Romance / Custom Gift Scam):**
  * *Scenario:* A victim is groomed by an online persona who claims to have sent expensive jewelry that is stuck in Delhi airport customs, requiring ₹35,000 clearance.
  * *Interception:* Before confirmation, the app prompts: *"Record a 10-second voice note explaining who this person is and why you are transferring funds."*
  * *Outcome:* The victim speaks aloud: *"I am sending ₹35,000 to clear a gift from someone I met on Instagram three weeks ago..."* Hearing the words spoken out loud triggers self-awareness of the absurdity, prompting the victim to abort.
...
* **Real-World Use Case (Stock Trading IPO Scam):**
  * *Scenario:* A victim believes they are transferring ₹2,00,000 to an authorized institutional brokerage account for pre-IPO allocation.
  * *Timeline Display:*
  ...
  * *Outcome:* The visual proof that the account is an active churn-and-drain mule instantly shatters the institutional broker illusion.
...
* **Real-World Use Case (Credit Card KYC Update Scam):**
  * *Scenario:* A scammer calls claiming to be from SBI Card Fraud Prevention, asking the victim to transfer ₹10,000 to "verify card security".
  * *Interception:* The app detects bank impersonation markers and displays: **[Call Official SBI Helpline (1800-1234)]**.
  * *Outcome:* The user taps the button, connects to genuine SBI support, and is informed that no such call was initiated by the bank.

---

### 31. Post-Payment Regret Window (Escrow Hold Vault)
* **Technical Mechanism (How It Works):** 
  Introduces an active 15-minute escrow or delayed-settlement window for flagged payments, allowing the user to reverse the transaction with a single tap after experiencing post-call clarity.
* **Real-World Use Case (Post-Call Panic Reversal):**
  * *Scenario:* A victim in a state of panic overrides warnings and transfers ₹30,000 for a fake job deposit. The moment the transfer completes, the scammer abruptly disconnects and blocks the victim.
  * *Recovery:* The victim realizes they have been defrauded. The payment screen maintains an active timer: **[Tap to Cancel & Recall Funds (12:44 remaining)]**. The victim taps the recall button, halting settlement before the funds leave the settlement buffer.
...
* **Real-World Use Case (Multi-App Coerced Splitting):**
  * *Scenario:* Scammer instructs victim: *"Send ₹15,000 from GPay, ₹15,000 from PhonePe, and ₹15,000 from Paytm to `clearance@axis`."*
  * *Detection:* GPay sees one ₹15,000 transfer (normal). PhonePe sees one ₹15,000 transfer (normal). Paytm sees one ₹15,000 transfer (normal). Kurukshetra's MCP server correlates the sender and recipient across all three apps within a 15-minute window, identifying a composite ₹45,000 smurfing attack.
  * *Interception:* Kurukshetra blocks the 2nd and 3rd legs of the transaction across all participating apps.
...
* **Real-World Use Case (Mass Electricity Phishing Outbreak):**
  * *Scenario:* At 6:00 PM, a cyber syndicate sends 100,000 SMS messages across India regarding fake electricity disconnections, directing victims to `quickbill.desk@icici`.
  * *Detection:* Within 35 minutes, 28 victims across 6 different banks attempt payments to this ID. Kurukshetra identifies the campaign signature across the MCP network.
  * *Interception:* An automated campaign termination order is broadcast to every UPI application, neutralizing the VPA nationwide within 40 minutes of the first victim report.
...
* **Real-World Use Case (Pre-Payment Family Safety Check):**
  * *Scenario:* An elderly mother receives a WhatsApp message from a "temple trust" requesting a ₹5,000 donation to `darshan.seva@kotak`. She asks her son to check if it is safe.
  * *Action:* The son enters `darshan.seva@kotak` into `kurukshetra.gov.in/lookup`.
  * *Result:* The portal displays: *Risk Score: 96/100 (Extremely High). Account opened 3 days ago. 22 past abandonments. 0 religious trust verification.* The family avoids losing funds.
...
* **Real-World Use Case (Coached Cooling-Off Bypass):**
  * *Scenario:* A user's ₹25,000 transfer is held for 4 hours. The scammer tells the victim: *"The bank is testing you. Wait 4 hours, confirm it, and immediately send the remaining ₹75,000."*
  * *Detection:* Exactly at hour 4, the user confirms the ₹25,000 payment and within 60 seconds initiates a new ₹75,000 payment to the same recipient.
  * *Interception:* Kurukshetra flags the post-hold immediate escalation as an active coaching signature and escalates to mandatory operator intervention.
...
* **Real-World Use Case (A/B Testing Coaching Scripts):**
  * *Scenario:* Kurukshetra tests two approaches for Digital Arrest interception:
  ...
  * *System Optimization:* The system updates its routing policy to deploy Script B across all digital arrest scenarios, maximizing life-saving intervention rates.
...
| 25 | H. Interventions | Scam Playbook Intervention | Pre-PIN Visual Narrative Stepper | Digital Arrest, Customs Coercion |
...
| 28 | H. Interventions | Voice Explanation Challenge | Audio Articulation / Cognitive Debiasing| Romance Scams, Honeytraps |
...
| 31 | H. Interventions | Post-Payment Regret Window | 15-Minute Escrow Reversal Buffer | Post-Panic Clarity Recovery |
