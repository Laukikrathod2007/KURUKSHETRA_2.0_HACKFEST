...
export default function App() {
  // ---- View / mode state -------------------------------------------------
  const [viewMode, setViewMode] = useState("mode-dual");
  ...
  // ---- 3D Universe embed source -----------------------------------------------
  const [universeUrl, setUniverseUrl] = useState("http://localhost:5174/");
  ...
  function setAuthenticatedUser(user) {
    setCurrentUser(user);
    // Citizens are locked to phone-only mode; moderators get full dual view.
    if (user.role === "moderator") {
      setViewMode("mode-dual");
    } else {
      setViewMode("mode-gpay");
    }
    setScreen("home");
  }
  ...
  function doLogin() {
    postJson("/auth/login", { username: loginUser.trim(), password: loginPass.trim() })
      .then(async (res) => {
        if (!res.ok) {
          alert("Invalid credentials.\n\nCitizen: android1 / 1234\nModerator: moderator1 / 4321");
          return;
        }
  ...
  async function refreshSocBalances() {
    try {
      ...
      setSocAccounts(Array.isArray(accounts) ? accounts : []);
    } catch (e) {
      console.error(e);
    }
  }
  ...
  function openPaymentComposer(payee) {
    ...
    getJson(`/public/lookup/${encodeURIComponent(payee.vpa)}`)
      .then((data) => {
        // Citizen-facing: terse status only, no internal metrics.
        if (data.mismatch_warning) {
          setComposerAudit({ badgeClass: "badge-warn", badge: "⚠ CAUTION", text: "Beneficiary account type does not match stated purpose." });
        } else if (data.mule_warning) {
          setComposerAudit({ badgeClass: "badge-danger", badge: "🛑 BLOCKED", text: "This VPA has been flagged. Transaction not permitted." });
        } else if (data.merchant_badge) {
          setComposerAudit({ badgeClass: "", badge: "✓ VERIFIED", text: "Registered merchant. Cleared for payment." });
        } else if (data.risk_level === "SAFE") {
          setComposerAudit({ badgeClass: "", badge: "✓ CLEARED", text: "No adverse indicators on file." });
        } else {
          setComposerAudit({ badgeClass: "badge-warn", badge: "⚠ REVIEW", text: "Unverified beneficiary. Proceed with caution." });
        }
      })
      .catch(() => {
        setComposerAudit({ badgeClass: "", badge: "OFFLINE", text: "Registry check unavailable." });
      });
  }
  ...
  async function runLiveAudit(vpa) {
    setLiveAudit({ status: "CHECKING...", statusClass: "", detailHtml: "", meta: "" });
    try {
      const data = await getJson(`/public/lookup/${encodeURIComponent(vpa)}`);
      const meta = `Registered to: ${data.resolved_name} · ${data.account_type}`;

      if (data.mismatch_warning) {
        setLiveAudit({ status: "⚠ CAUTION — ACCOUNT MISMATCH", statusClass: "status-amber", detailHtml: "Account type does not correspond to the stated payment purpose. Verify before proceeding.", detailColor: "#fdba74", meta, proceedLabel: "Acknowledge and Proceed →", resolvedName: data.resolved_name });
      } else if (data.mule_warning) {
        setLiveAudit({ status: "🛑 BLOCKED — FLAGGED ACCOUNT", statusClass: "status-red", detailHtml: "This UPI ID has been flagged. Payment is not permitted.", detailColor: "#fca5a5", meta, proceedLabel: null, resolvedName: data.resolved_name });
      } else if (data.merchant_badge) {
        setLiveAudit({ status: "✓ VERIFIED MERCHANT", statusClass: "status-green", detailHtml: "Registered and verified merchant account. Cleared for payment.", detailColor: "#6ee7b7", meta, proceedLabel: "Pay →", resolvedName: data.resolved_name });
      } else if (data.risk_level === "SAFE") {
        setLiveAudit({ status: "✓ CLEARED", statusClass: "status-green", detailHtml: "No adverse indicators. Cleared for payment.", meta, proceedLabel: "Pay →", resolvedName: data.resolved_name });
      } else {
        setLiveAudit({ status: "⚠ UNVERIFIED BENEFICIARY", statusClass: "status-amber", detailHtml: "No prior history on file. Confirm recipient identity before proceeding.", meta, proceedLabel: "Acknowledge and Proceed →", resolvedName: data.resolved_name });
      }
    } catch (e) {
      console.error(e);
      setLiveAudit({ status: "OFFLINE", statusClass: "", detailHtml: "Registry check unavailable.", meta: "" });
    }
  }
  ...
  async function runScenario(scen) {
    // Clear any overlay left on screen from a previous scenario click --
    // otherwise, e.g., RED's hard-block sheet stays visible after switching
    // straight to YELLOW, since YELLOW never itself opens/closes a sheet.
    setFreezeSheet(null);
    setCoachSheet(null);
    ...
    try {
      ...
      if (data.decision) {
        ...
        setSocRisk({
          ...
          audit_ref: "1",
          signals: data.reasons ? data.reasons.map((r, i) => ({ feature_id: i + 1, feature_name: r, label: r, severity: "CRITICAL", risk_contribution: 0.25, triggered: true })) : [],
          intervention_screen: data.intervention_screen,
        });
      }
      ...
      if (scen === "green") {
        ...
        showCoachingSheet(
          data.intervention_screen || data.decision?.intervention_screen || { warning_title: "Confirm New Recipient", explanation: "This is a first-time payment to an unverified merchant. Please confirm you recognize this recipient before proceeding." }
        );
      } else if (scen === "orange") {
        showHardBlockSheet(
          data.intervention_screen || data.decision?.intervention_screen || { warning_title: "Payment Blocked: CBI Extortion", explanation: "Declared purpose contradicts recipient savings account. Coercive extortion scheme detected." }
        );
      } else if (scen === "red") {
        showHardBlockSheet(
          data.intervention_screen || data.decision?.intervention_screen || { warning_title: "Payment Blocked: Mule Syndicate", explanation: "Account exhibits rapid drain velocity, multi-reporter flags, and high abandonment." }
        );
      }
    } catch (e) {
      console.error(e);
  ...
  const isCitizen = !isModerator;

  // Full-screen pre-app login gate — shown before any chrome when no session
  if (screen === "login") {
    return (
      <FullScreenLoginGate
        username={loginUser}
        ...
        onAutofillCitizen={() => { setLoginUser("android1"); setLoginPass("1234"); }}
        onAutofillModerator={() => { setLoginUser("moderator1"); setLoginPass("4321"); }}
        onLogin={doLogin}
      />
    );
  }
    ...
    <>
      <header className="global-navbar">
        ...
        <div className="nav-center view-mode-selector">
          {isCitizen ? (
            <>
              <button className="mode-toggle-btn active" style={{ display: "inline-flex", alignItems: "center", gap: 6, cursor: "default", opacity: 1 }}>
                <Smartphone size={14} /> Citizen UPI App
              </button>
              <span className="citizen-role-badge" style={{ display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11, color: "#6ee7b7", fontWeight: 600, letterSpacing: "0.08em", padding: "2px 10px", background: "rgba(110,231,183,0.10)", borderRadius: 20, border: "1px solid rgba(110,231,183,0.25)" }}>
                <ShieldCheck size={12} /> CITIZEN SESSION
              </span>
            </>
          ) : (
            <>
              <button className={`mode-toggle-btn${viewMode === "mode-gpay" ? " active" : ""}`} onClick={() => setViewMode("mode-gpay")} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                <Smartphone size={14} /> GPay App
              </button>
              <button className={`mode-toggle-btn${viewMode === "mode-dual" ? " active" : ""}`} onClick={() => setViewMode("mode-dual")} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                <Monitor size={14} /> Dual View (App + SOC)
              </button>
              <button className={`mode-toggle-btn${viewMode === "mode-soc" ? " active" : ""}`} onClick={() => setViewMode("mode-soc")} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                <Shield size={14} /> Ecosystem SOC
              </button>
              <button className={`mode-toggle-btn${viewMode === "mode-universe" ? " active" : ""}`} onClick={() => setViewMode("mode-universe")} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                🌐 Live Universe
              </button>
            </>
          )}
        ...
        <div className="nav-right">
          <button className="btn-tool-pill" title="Open Public Scam Score Portal (Feature #34)" onClick={openCitizenChecker} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
            <Shield size={14} /> Citizen Scam Check
          </button>
          {isModerator && (
            <>
              <div className="metric-chip" id="audit-chip" title="Click to verify SHA-256 hash-chain integrity" onClick={verifyAuditChain}>
                <span className="icon" style={{ display: "inline-flex" }}><Link2 size={14} /></span>
                <span>AUDIT: <strong>VERIFIED</strong></span>
              </div>
              <button className="btn-reset-pill" title="Reset all bank accounts and database to seed" onClick={resetDb} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                <RefreshCw size={14} /> Reset
              </button>
            </>
          )}
          <button className="btn-reset-pill" style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "rgba(248,113,113,0.10)", borderColor: "rgba(248,113,113,0.3)", color: "#fca5a5" }} onClick={logout} title="Logout">
            ✕ Logout ({currentUser?.name?.split(" ")[0] || "User"})
          </button>
        </div>
      </header>

      <div className="main-viewport-container">
        {/* Universe mode: full-screen iframe embed of the 3D Particle Universe */}
        {viewMode === "mode-universe" && (
          <div className="universe-iframe-panel">
            <div className="universe-iframe-header">
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.1em", color: "#38bdf8" }}>🌐 KURUKSHETRA LIVE ENTITY UNIVERSE</span>
              <span style={{ fontSize: 11, color: "rgba(148,163,184,0.6)" }}>Real-time 3D graph • 43k entities • Live SSE sync</span>
              <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
                <button
                  type="button"
                  onClick={() => setUniverseUrl("http://localhost:5174/")}
                  style={{
                    fontSize: 11, padding: "3px 10px", borderRadius: 4,
                    background: universeUrl.includes("5174") ? "rgba(56,189,248,0.25)" : "transparent",
                    color: universeUrl.includes("5174") ? "#38bdf8" : "#94a3b8",
                    border: "1px solid rgba(56,189,248,0.3)", cursor: "pointer",
                    fontWeight: universeUrl.includes("5174") ? 700 : 500,
                  }}
                  title="Connect directly to Vite Dev Server (port 5174)"
                >
                  ⚡ Vite Dev (:5174)
                </button>
                <button
                  type="button"
                  onClick={() => setUniverseUrl("http://localhost:8000/universe/")}
                  style={{
                    fontSize: 11, padding: "3px 10px", borderRadius: 4,
                    background: universeUrl.includes("8000") ? "rgba(56,189,248,0.25)" : "transparent",
                    color: universeUrl.includes("8000") ? "#38bdf8" : "#94a3b8",
                    border: "1px solid rgba(56,189,248,0.3)", cursor: "pointer",
                    fontWeight: universeUrl.includes("8000") ? 700 : 500,
                  }}
                  title="Serve from FastAPI Production static mount (port 8000)"
                >
                  🏛️ FastAPI Built (:8000/universe)
                </button>
                <a
                  href={universeUrl}
                  target="_blank"
                  rel="noreferrer"
                  style={{ fontSize: 11, color: "#38bdf8", textDecoration: "none", padding: "3px 8px", fontWeight: 600 }}
            ...
            <iframe
              key={universeUrl}
              src={universeUrl}
              title="Kurukshetra Live Universe"
        ...
        <div className="gpay-phone-wrapper" style={viewMode === "mode-universe" ? { display: "none" } : undefined}>
          <div className="phone-app-rail-switcher">
            <button className={`rail-tab-btn${activeRail === "gpay" ? " active" : ""}`} onClick={() => switchAppRail("gpay")} style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
              <CircleDot size={12} style={{ color: "#4285F4" }} /> Google Pay
            </button>
            <button className={`rail-tab-btn${activeRail === "phonepe" ? " active" : ""}`} onClick={() => switchAppRail("phonepe")} style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
              <CircleDot size={12} style={{ color: "#9333ea" }} /> PhonePe
            </button>
            <button className={`rail-tab-btn${activeRail === "netbanking" ? " active" : ""}`} onClick={() => switchAppRail("netbanking")} style={{ display: "inline-flex", alignItems: "center", gap: 5 }}>
              <Landmark size={12} /> SBI NetBanking
            </button>
          </div>
          ...
          <div className="phone-device-bezel">
            {toast && (
              <div className="trusted-alert-toast">
                <div className="toast-bell"><Bell size={18} /></div>
            ...
            <div className="phone-screen-inner">
              ...
              {screen === "home" && (
                <GPayHome
                  ...
                  onOpenScamCheck={openCitizenChecker}
                />
              )}
...
// =============================================================================
// Full-screen pre-app login gate (shown before any chrome)
// =============================================================================

function FullScreenLoginGate({ username, password, onUsername, onPassword, onAutofillCitizen, onAutofillModerator, onLogin }) {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          position: "absolute", top: "15%", left: "25%", width: 480, height: 480,
          background: "radial-gradient(ellipse, rgba(56,189,248,0.07) 0%, transparent 70%)",
          borderRadius: "50%", filter: "blur(30px)",
        }} />
        <div style={{
          position: "absolute", bottom: "20%", right: "20%", width: 380, height: 380,
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{
            ...
            <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.15em", color: "#38bdf8" }}>KURUKSHETRA // 2.0</span>
          </div>
          <h1 style={{ margin: 0, fontSize: 28, fontWeight: 800, color: "#f8fafc", letterSpacing: "-0.02em" }}>
            Agentic Guardian
          </h1>
          <p style={{ margin: "8px 0 0", fontSize: 14, color: "rgba(148,163,184,0.8)", lineHeight: 1.5 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 28 }}>
          <button
            onClick={onAutofillCitizen}
            style={{
              background: "rgba(52,211,153,0.06)", border: "1px solid rgba(52,211,153,0.2)",
              borderRadius: 12, padding: "12px 14px", textAlign: "left", cursor: "pointer",
              transition: "all 0.2s",
            }}
            onMouseEnter={e => { e.currentTarget.style.background = "rgba(52,211,153,0.12)"; e.currentTarget.style.borderColor = "rgba(52,211,153,0.4)"; }}
          ...
          >
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", color: "#34d399", marginBottom: 5 }}>
              📱 CITIZEN
            </div>
            <div style={{ fontSize: 12, color: "rgba(226,232,240,0.7)" }}>
              <code style={{ background: "rgba(0,0,0,0.3)", padding: "1px 5px", borderRadius: 4, fontSize: 11 }}>android1</code>
              {" / "}
              <code style={{ background: "rgba(0,0,0,0.3)", padding: "1px 5px", borderRadius: 4, fontSize: 11 }}>1234</code>
            </div>
            <div style={{ fontSize: 10, color: "rgba(148,163,184,0.5)", marginTop: 4 }}>Click to auto-fill</div>
          </button>
          <button
            onClick={onAutofillModerator}
            style={{
          ...
          >
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", color: "#a78bfa", marginBottom: 5 }}>
              🛡 MODERATOR
            </div>
            <div style={{ fontSize: 12, color: "rgba(226,232,240,0.7)" }}>
              <code style={{ background: "rgba(0,0,0,0.3)", padding: "1px 5px", borderRadius: 4, fontSize: 11 }}>moderator1</code>
              {" / "}
              <code style={{ background: "rgba(0,0,0,0.3)", padding: "1px 5px", borderRadius: 4, fontSize: 11 }}>4321</code>
            </div>
            <div style={{ fontSize: 10, color: "rgba(148,163,184,0.5)", marginTop: 4 }}>Click to auto-fill</div>
          </button>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div>
            <label style={{ display: "block", fontSize: 11, fontWeight: 600, color: "rgba(148,163,184,0.8)", letterSpacing: "0.08em", marginBottom: 7 }}>
              USERNAME
            </label>
            <input
              id="gate-username"
              ...
              placeholder="android1 or moderator1"
              style={{
                width: "100%", boxSizing: "border-box",
                background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)",
                borderRadius: 10, padding: "12px 16px", color: "#f8fafc", fontSize: 14,
                outline: "none", fontFamily: "inherit",
                transition: "border-color 0.2s",
              }}
              onFocus={e => { e.target.style.borderColor = "rgba(56,189,248,0.5)"; e.target.style.boxShadow = "0 0 0 3px rgba(56,189,248,0.07)"; }}
        ...
        {/* Footer */}
        <div style={{ textAlign: "center", marginTop: 24, fontSize: 11, color: "rgba(100,116,139,0.7)", lineHeight: 1.6 }}>
          <Lock size={11} style={{ display: "inline", verticalAlign: "middle", marginRight: 4 }} />
          Secured by Kurukshetra Real-Time Fraud Interception Engine · PS09 Demo
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="gpay-view" id="screen-login">
      ...
      <div className="login-footer-security">
        <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><Lock size={13} /> Protected by Kurukshetra Real-Time ML/Heuristic Interception</span>
      </div>
    </div>
  );
}
...
function GPayHome({ currentUser, bank, onOpenThread, onOpenPayUpi, onOpenQr, onBankTransfer, onCheckBalance, onViewHistory, onOpenProfile, onOpenScamCheck }) {
    ...
    <div className="gpay-view" id="screen-gpay-home">
      <div className="gpay-home-header">
        <div className="gpay-search-bar">
          ...
          <span className="mic-icon"><Mic size={16} /></span>
        </div>
      ...
      <div className="gpay-home-scroll">
        <div className="gpay-actions-grid">
          <div className="gpay-action-item" onClick={onOpenQr}>
            <div className="action-icon-circle blue-grad"><Camera size={20} /></div>
          ...
          <div className="gpay-action-item" onClick={() => onOpenThread({ name: "Suresh Kirana Store", vpa: "grocer.local@oksbi", amount: 450, note: "Groceries" })}>
            <div className="action-icon-circle blue-grad"><Users size={20} /></div>
            <span className="action-label">Pay contacts</span>
          </div>
          <div className="gpay-action-item" onClick={onOpenPayUpi}>
            <div className="action-icon-circle blue-grad"><Smartphone size={20} /></div>
            <span className="action-label">Pay phone</span>
          </div>
          <div className="gpay-action-item" onClick={onBankTransfer}>
            <div className="action-icon-circle blue-grad"><Landmark size={20} /></div>
            <span className="action-label">Bank transfer</span>
          </div>
          <div className="gpay-action-item" onClick={onOpenPayUpi}>
            <div className="action-icon-circle blue-grad"><IdCard size={20} /></div>
            <span className="action-label">Pay UPI ID</span>
          </div>
          <div className="gpay-action-item">
            <div className="action-icon-circle blue-grad"><Repeat size={20} /></div>
            <span className="action-label">Self transfer</span>
          </div>
          <div className="gpay-action-item">
            <div className="action-icon-circle blue-grad"><Lightbulb size={20} /></div>
            <span className="action-label">Pay bills</span>
          </div>
          <div className="gpay-action-item">
            <div className="action-icon-circle blue-grad"><Wifi size={20} /></div>
            ...
            {PEOPLE.map((p) => (
              <div className="person-contact-item" key={p.vpa} onClick={() => onOpenThread({ name: p.name, vpa: p.vpa, amount: p.amount, note: p.note })}>
                <div className={`person-avatar-circle ${p.cls}`}>
                  ...
                  {p.badge === "safe" && <div className="safe-badge-tick"><Check size={10} /></div>}
                  {p.badge === "alert" && <div className="alert-badge-icon"><AlertTriangle size={10} /></div>}
                  {p.badge === "danger" && <div className="danger-badge-icon"><TriangleAlert size={10} /></div>}
                </div>
        ...
        <div className="gpay-section-container">
          ...
          <div className="manage-money-card" onClick={onCheckBalance}>
            <div className="money-card-icon"><Landmark size={18} /></div>
            ...
            <div className="money-card-arrow"><ChevronRight size={16} /></div>
          </div>

          <div className="manage-money-card" onClick={onViewHistory}>
            <div className="money-card-icon"><Receipt size={18} /></div>
            <div className="money-card-info">
          ...
          <div className="manage-money-card" onClick={onOpenScamCheck} style={{ borderColor: "rgba(56,189,248,0.25)", background: "rgba(56,189,248,0.04)" }}>
            <div className="money-card-icon" style={{ color: "#38bdf8" }}>🔍</div>
            <div className="money-card-info">
              <div className="money-card-title" style={{ color: "#38bdf8" }}>Check any UPI ID for scams</div>
              <div className="money-card-sub">Public safety lookup — no login required</div>
            </div>
            <div className="money-card-arrow">&rarr;</div>
          </div>
        </div>

        {/* Demo Scenarios — surfaces the real risk engine */}
        <div className="gpay-section-container demo-scenarios-section">
          <div className="section-heading-row">
            <span className="section-title">🧪 Live Demo Scenarios</span>
            <span className="section-sub" style={{ fontSize: 10, color: "rgba(148,163,184,0.5)", fontWeight: 400 }}>Each fires the real Kurukshetra FRM</span>
          </div>
          <div className="demo-scenario-list">
            <div className="demo-scenario-row" onClick={() => onOpenThread({ name: "Suresh Kirana Store", vpa: "grocer.local@oksbi", amount: 450, note: "Groceries" })}>
              <span className="demo-scenario-dot" style={{ background: "#34d399" }} />
              <div className="demo-scenario-info">
                <div className="demo-scenario-name">Trusted Grocer Payment</div>
                <div className="demo-scenario-desc">Regular merchant — Tier 0 clears in &lt;10ms</div>
              </div>
              <span className="demo-outcome-badge" style={{ color: "#34d399", borderColor: "rgba(52,211,153,0.3)", background: "rgba(52,211,153,0.08)" }}>✓ ALLOW</span>
            </div>
            <div className="demo-scenario-row" onClick={() => onOpenThread({ name: "Unregistered New Shop", vpa: "newshop.mumbai@oksbi", amount: 2500, note: "Apparel" })}>
              <span className="demo-scenario-dot" style={{ background: "#fbbf24" }} />
              <div className="demo-scenario-info">
                <div className="demo-scenario-name">New Unverified Merchant</div>
                <div className="demo-scenario-desc">No history on file — Step-up confirmation triggered</div>
              </div>
              <span className="demo-outcome-badge" style={{ color: "#fbbf24", borderColor: "rgba(251,191,36,0.3)", background: "rgba(251,191,36,0.08)" }}>⚠ STEP-UP</span>
            </div>
            <div className="demo-scenario-row" onClick={() => onOpenThread({ name: "Manoj Kumar (Fake CBI)", vpa: "cbi.clearance.cell@sbi", amount: 75000, note: "GOVT_FINE" })}>
              <span className="demo-scenario-dot" style={{ background: "#f97316" }} />
              <div className="demo-scenario-info">
                <div className="demo-scenario-name">Fake CBI Officer Extortion</div>
                <div className="demo-scenario-desc">Authority handle → personal savings account mismatch</div>
              </div>
              <span className="demo-outcome-badge" style={{ color: "#f97316", borderColor: "rgba(249,115,22,0.3)", background: "rgba(249,115,22,0.08)" }}>🛑 BLOCKED</span>
            </div>
            <div className="demo-scenario-row" onClick={() => onOpenThread({ name: "Deepak Layering Account", vpa: "mule.syndicate@axis", amount: 50000, note: "Crypto Settlement" })}>
              <span className="demo-scenario-dot" style={{ background: "#f87171" }} />
              <div className="demo-scenario-info">
                <div className="demo-scenario-name">Mule Syndicate Pass-Through</div>
                <div className="demo-scenario-desc">5 independent signals — CBS forensics + community reports</div>
              </div>
              <span className="demo-outcome-badge" style={{ color: "#f87171", borderColor: "rgba(248,113,113,0.3)", background: "rgba(248,113,113,0.08)" }}>🛑 FREEZE</span>
            </div>
            <div className="demo-scenario-row" onClick={onOpenQr}>
              <span className="demo-scenario-dot" style={{ background: "#a78bfa" }} />
              <div className="demo-scenario-info">
                <div className="demo-scenario-name">QR Code Phishing Trap</div>
                <div className="demo-scenario-desc">Scan-to-receive QR that actually debits your account</div>
              </div>
              <span className="demo-outcome-badge" style={{ color: "#a78bfa", borderColor: "rgba(167,139,250,0.3)", background: "rgba(167,139,250,0.08)" }}>🛑 BLOCKED</span>
            </div>
          </div>
        </div>
      ...
      <div className="sheet-content sheet-warning">
        <div className="sheet-icon-header">
          ...
          <div style={{ flex: 1 }}>
            ...
            <div className="sheet-ai-badge" style={{ fontSize: 10, letterSpacing: "0.08em" }}>KURUKSHETRA FRM · REF: {sheet.ref || "KSH-COACH"}</div>
          </div>
        </div>

        <div className="sheet-explanation" style={{ fontWeight: 500, letterSpacing: "0.01em" }}>
          This payment has been flagged and placed under caution hold.
          Verify the identity of the recipient before authorising.
          Do not proceed if you are under instruction from a third party.
        </div>

        <label className="sheet-ack-checkbox">
          <input type="checkbox" checked={ack} onChange={(e) => onAck(e.target.checked)} />
          <span>I have independently verified the recipient and choose to proceed.</span>
        </label>
    ...
    <div className="material-bottom-sheet" id="sheet-freeze">
      <div className="sheet-content sheet-danger">
        <div className="sheet-icon-header">
          ...
          <div style={{ flex: 1 }}>
            <div className="sheet-title">{sheet.title}</div>
            <div className="sheet-ai-badge" style={{ fontSize: 10, letterSpacing: "0.08em" }}>KURUKSHETRA FRM · FREEZE ORDER · REF: {sheet.ref || "KSH-FREEZE"}</div>
          </div>
        </div>

        <div className="sheet-explanation" style={{ fontWeight: 600, letterSpacing: "0.015em" }}>
          PAYMENT INTERCEPTED.<br />
          This transaction has been blocked by the fraud interception system.
          No funds have been debited from your account.
        </div>

        <div className="sheet-safe-guarantee">
          🛡️ <strong>Zero funds moved.</strong> Your account balance is unchanged.
        </div>

        <div className="sheet-actions">
          <button className="btn-sheet-helpline" onClick={onHelpline}>📞 Call 1930 Cyber Helpline</button>
          <button className="btn-sheet-cancel" onClick={onDismiss}>Acknowledged</button>
        </div>
      </div>
    </div>
  );
            ...
            <input
              ...
              onPaste={(e) => {
                ...
                if (pasted) {
                  onChange(pasted);
                  if (pasted.includes("@")) {
                    setTimeout(() => onAudit(pasted), 60);
                  }
                }
              }}
            />
            <button type="button" className="btn-input-audit" onClick={() => value.trim() && onAudit(value.trim())}>Check →</button>
          </div>
...
// =============================================================================
// Operations Console (Right Side) — Premium Financial Intelligence Surface
// =============================================================================

function SocObserver({ activeScenario, onRunScenario, onSurprise, narrative, activeNodes, accounts, risk, traceId, traces, campaigns, onKillSwitch }) {
  ...
  const intervention = risk?.intervention_screen;
  const explanation = intervention?.explanation;

  const nodes = [
    { id: "snode-payer", title: "Payer Client", sub: "Aarav Sharma" },
    { id: "snode-switch", title: "NPCI Switch", sub: "Central Mapper" },
    { id: "snode-kurukshetra", title: "Kurukshetra FRM", sub: "Tier 0 · Tier 1", isEngine: true },
    { id: "snode-remitter", title: "Remitter CBS", sub: "SBI Core Banking" },
    { id: "snode-beneficiary", title: "Beneficiary CBS", sub: "Settlement Bank" },
  ];

  return (
    <div className="soc-console">
      {/* 1. Simulation Presets Bar */}
      <div className="console-scenario-bar">
        <span className="scenario-caption">Simulation Presets</span>
        <div className="scenario-buttons-track">
          <button
            className={`scenario-btn ${activeScenario === "green" ? "active" : ""}`}
            onClick={() => onRunScenario("green")}
          >
            <span className="dot dot-allow"></span>
            <span>Trusted Grocer</span>
          </button>
          <button
            className={`scenario-btn ${activeScenario === "yellow" ? "active" : ""}`}
            onClick={() => onRunScenario("yellow")}
          >
            <span className="dot dot-stepup"></span>
            <span>Unverified Shop</span>
          </button>
          <button
            className={`scenario-btn ${activeScenario === "orange" ? "active" : ""}`}
            onClick={() => onRunScenario("orange")}
          >
            <span className="dot dot-coach"></span>
            <span>Law Impersonation</span>
          </button>
          <button
            className={`scenario-btn ${activeScenario === "red" ? "active" : ""}`}
            onClick={() => onRunScenario("red")}
          >
            <span className="dot dot-freeze"></span>
            <span>Mule Syndicate</span>
          </button>
          <button
            className={`scenario-btn ${activeScenario === "blue" ? "active" : ""}`}
            onClick={() => onRunScenario("blue")}
          >
            <span className="dot dot-card"></span>
            <span>Card CNP (3DS)</span>
          </button>
          <button
            className={`scenario-btn surprise ${activeScenario === "surprise" ? "active" : ""}`}
            onClick={onSurprise}
          >
            <span>✨</span>
            <span>Adversary Probe</span>
          </button>
        </div>
      </div>

      {/* 2. Active Briefing / Narrative */}
      {narrative && (
        <div className={`console-briefing ${narrative.zoneClass}`}>
          <div className="briefing-title">{narrative.title}</div>
          <p className="briefing-body">{narrative.body}</p>
          <div className="briefing-meta">{narrative.meta}</div>
        </div>
      )}

      {/* 3. Transaction Pipeline Flow */}
      <section className="console-section">
        <div className="section-header-quiet">
          <span>Transaction Pipeline Architecture</span>
          <span className="status-indicator-live">Live</span>
        </div>
        <div className="pipeline-flow-track">
          {nodes.map((n, i) => (
            <div className="pipeline-step-unit" key={n.id}>
              <div className={`pipeline-node ${n.isEngine ? "engine" : ""} ${activeNodes?.has ? (activeNodes.has(n.id) ? "active" : "") : ""}`}>
                <div className="pipeline-node-name">{n.title}</div>
                <div className="pipeline-node-meta">{n.sub}</div>
              </div>
              {i < nodes.length - 1 && <div className="pipeline-connector-line"></div>}
            </div>
          ))}
        </div>
      </section>

      {/* 4. Core Banking Ledgers */}
      <section className="console-section">
        <div className="section-header-quiet">
          <span>Core Banking System (CBS) Ledgers</span>
          <span className="section-header-tag">Double-Entry Verified</span>
        </div>
        <div className="ledger-accounts-strip">
          {safeAccounts.map((acc) => (
            <div className="ledger-unit" key={acc.account_id}>
              <div className="ledger-holder">{acc.customer_name}</div>
              <div className="ledger-balance">{acc.balance_formatted}</div>
              <div className="ledger-meta">{acc.bank_id} · {acc.account_number}</div>
            </div>
          ))}
        </div>
      </section>

      {/* 5. Risk Engine Forensics */}
      <section className="console-section">
        <div className="section-header-quiet">
          <span>Kurukshetra Risk Engine Forensics</span>
          <span className={`verdict-text-inline ${zone.toLowerCase()}`}>{zone}</span>
        </div>

        <div className="forensics-hero-block">
          <div className="verdict-primary-display">
            <div className="verdict-label">DECISION VERDICT</div>
            <div className={`verdict-score-figure ${zone.toLowerCase()}`}>
              {risk?.risk_score !== undefined ? risk.risk_score.toFixed(2) : "0.00"}
              <span className="verdict-scale"> / 1.00</span>
            </div>
            <div className="verdict-subtitle">
              {!risk || risk.tier_reached === 0
                ? "Tier 0 Fast-Path · Sub-10ms In-Memory Analysis"
                : `Tier ${risk.tier_reached} Deep Forensics Invoked`}
            </div>
          </div>

          <div className="metrics-summary-strip">
            <div className="metric-unit">
              <span className="metric-caption">Policy Action</span>
              <span className="metric-value">{risk?.decision || "PROCEED"}</span>
            </div>
            <div className="metric-unit">
              <span className="metric-caption">Completeness</span>
              <span className="metric-value">{risk?.data_completeness || "FULL"}</span>
            </div>
            <div className="metric-unit">
              <span className="metric-caption">Audit Sequence</span>
              <span className="metric-value">{risk?.audit_ref ? `#${risk.audit_ref}` : "PENDING"}</span>
            </div>
            <div className="metric-unit">
              <span className="metric-caption">Decision Tier</span>
              <span className="metric-value">{risk?.tier_reached ? `Tier ${risk.tier_reached}` : "Tier 0"}</span>
            </div>
          </div>
        </div>

        {/* Agentic Guardian LLM Synthesis Block */}
        {explanation && (
          <div className={`guardian-llm-banner ${zone.toLowerCase()}`}>
            <div className="llm-banner-header">
              <span className="llm-sparkle">✨</span>
              <span className="llm-banner-title">AGENTIC GUARDIAN LLM INTERVENTION</span>
              <span className="llm-model-tag">Gemini 3.1 Flash-Lite · Real-Time Anti-Coercion Protocol</span>
            </div>
            <p className="llm-explanation-text">{explanation}</p>
            {intervention?.trusted_contact_notification && (
              <div className="llm-trusted-notice">
                <span>📱 Trusted Contact Dispatched:</span> <em>{intervention.trusted_contact_notification.message}</em>
              </div>
            )}
          </div>
        )}
        ...
        <div className="signals-table-container">
          <table className="signals-table">
            ...
            <tbody>
              {signals.length === 0 ? (
                <tr>
                  <td colSpan={5} className="empty-signals-note">
                    Tier 0 Fast-Path clear · Zero anomalies triggered across 42 in-memory baseline features
                  </td>
                </tr>
              ) : (
                signals.map((s, i) => (
                  <tr key={i} className={s.triggered ? "row-triggered" : ""}>
                    <td className="mono-cell">#{s.feature_id}</td>
                    <td>
                      <div className="signal-name">{s.feature_name}</div>
                      <div className="signal-label">{s.label}</div>
                    </td>
                    <td><span className={`severity-tag ${s.severity ? s.severity.toLowerCase() : "low"}`}>{s.severity || "LOW"}</span></td>
                    <td className="mono-cell">{s.risk_contribution > 0 ? `+${s.risk_contribution}` : "0.0"}</td>
                    <td>
                      {s.triggered ? (
                        <span className="signal-state-triggered">TRIGGERED</span>
                      ) : (
                        <span className="signal-state-quiet">QUIET</span>
                      )}
                    </td>
                  </tr>
                ))
      ...
      <section className="console-section">
        <div className="section-header-quiet">
          <span>Hop-by-Hop Execution Trace</span>
          <span className="trace-id-tag">{traceId}</span>
        </div>
        <div className="trace-stream-list">
          {safeTraces.length === 0 ? (
            <div className="empty-trace-note">Ready for transaction execution · Microsecond traces stream here</div>
          ) : (
            safeTraces.map((ev, i) => (
              <div className="trace-line-item" key={i}>
                <span className="trace-elapsed">+{ev.elapsed_ms || 0}ms</span>
                <span className="trace-component">{ev.component || "SYS"}</span>
                <span className="trace-action">{ev.action}</span>
                <span className="trace-summary">{ev.summary}</span>
              </div>
            ))
          )}
        </div>
      ...
      {/* 7. Central Switch Registry & Kill-Switch */}
      <section className="console-section">
        <div className="section-header-quiet">
          <span>Central Switch Registry & Kill-Switch</span>
          <span className="section-header-tag">Regulatory Layer</span>
        </div>
        <p className="section-description">
          Cross-PSP velocity metrics. Activating the central kill-switch revokes malicious VPAs across all member banks in &lt;50ms.
        </p>
          ...
          <table className="signals-table">
            ...
            <tbody>
              {safeCampaigns.length === 0 ? (
                <tr><td colSpan={7} className="empty-signals-note">No active suspicious campaigns in registry</td></tr>
              ) : (
                safeCampaigns.map((c) => (
                  <tr key={c.target_ref}>
                    <td className="mono-cell"><strong>{c.target_ref}</strong></td>
                    <td>{c.customer_name}</td>
                    <td className="mono-cell">{c.bank_id}</td>
                    <td className="mono-cell">{c.lookup_count} ({c.distinct_psp_count} PSPs)</td>
                    <td className="mono-cell">{(c.abandon_ratio * 100).toFixed(0)}%</td>
                    <td>
                      {c.is_suspended ? (
                    ...
                    <td>
                      {c.is_suspended ? (
                        <button className="btn-killswitch" disabled>REVOKED</button>
                      ) : (
                        <button className="btn-killswitch active" onClick={() => onKillSwitch(c.target_ref)}>
                          KILL-SWITCH
                        </button>
                      )}
                    </td>
