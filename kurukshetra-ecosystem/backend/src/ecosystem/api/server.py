...
def login(req: LoginRequest, session: Session = Depends(get_db)):
    ...
    if req.username == "moderator1" and req.password == "4321":
        return {
            "success": True,
            "token": "sess_moderator1_soc_jwt",
            "user": {
                "role": "moderator",
                "customer_id": "moderator1",
                "name": "Kurukshetra SOC Operator",
                "phone": "+919900000001",
                "account_id": None,
                "account_number": None,
                "bank_id": "NPCI",
                "bank_name": "NPCI Central Operations",
                "balance_paise": 0,
                "balance_formatted": "N/A",
                "vpa": "soc.operator@npci",
            },
        }
    raise HTTPException(status_code=401, detail="Invalid credentials. Use android1/1234 (Citizen) or moderator1/4321 (Moderator).")
