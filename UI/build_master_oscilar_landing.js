...
let updated = html.replace(/<title>.*?<\/title>/i, '<title>Agentic Guardian: AI Agents for Every Risk Decision (PS09 // KURUKSHETRA 2.0)</title>');
updated = updated.replace(/content="Tempo: The blockchain for stablecoin payments"/gi, 'content="Agentic Guardian: AI agents for every risk decision."');
updated = updated.replace(/Stripe and Paradigm/g, 'Team Midnight Ciphers (KURUKSHETRA 2.0)');

// 2. Hero Replacement (Image 1)
const heroContent = `<section class="_landingHero_golzh_1" style="padding-top: 40px; padding-bottom: 80px;">
  <div class="grid-container _heroContainer_golzh_16">
    ...
    <div class="grid-layout" style="align-items: center; gap: 32px;">
      <div class="col-6 md:col-6 lg:col-6">
        <h1 style="font-size: clamp(2.8rem, 5vw, 4.5rem); font-weight: 900; color: #000; line-height: 1.08; margin-bottom: 24px; letter-spacing: -0.03em;">
          AI agents for <br/>
          <span style="color: #ef4444;">every</span> risk decision.
        </h1>
        <p style="font-size: 16px; color: #475569; line-height: 1.7; margin-bottom: 28px; max-width: 540px;">
      ...
      <div class="col-6 md:col-6 lg:col-6">
        <div style="background: #ffffff; border: 1px solid rgba(0,0,0,0.08); border-radius: 24px; padding: 24px; box-shadow: 0 20px 40px rgba(0,0,0,0.06); position: relative;">
          <div style="background: #fafafa; border: 1px solid rgba(0,0,0,0.06); border-radius: 16px; padding: 18px; margin-bottom: 20px;">
          ...
          <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-bottom: 20px; border-bottom: 1px solid rgba(0,0,0,0.06); padding-bottom: 16px;">
            <div>
          ...
          <div style="border-radius: 12px; overflow: hidden; border: 1px solid rgba(0,0,0,0.08);">
            <img src="/oscilar_site/assets/framerusercontent.com/images/2uxHmtN3tfZFmf19Nwx5UKaEtY.png" alt="Oscilar Real-Time Risk Dashboard" style="width: 100%; height: 220px; object-fit: cover; display: block;" />
          </div>
        </div>
      </div>
    </div>
...
updated = updated.replace(/<section class="_landingHero_golzh_1">[\s\S]*?<\/section>/i, heroContent);

// 3. Section 1 Replacement (Purpose Built for Scam Interception)
const idx1 = updated.indexOf('data-section="animation-1"');
const idx2 = updated.indexOf('data-section="animation-2"');
const idx3 = updated.indexOf('data-section="animation-3"');
const idx4 = updated.indexOf('data-section="animation-4"');
...
// Construct new HTML
const part0 = updated.substring(0, idx1);
const part4 = updated.substring(idx4);

const finalHTML = part0 + section1Content + '\n' + section2Content + '\n' + section3Content + '\n' + part4;

fs.writeFileSync('index.html', finalHTML, 'utf8');
console.log('Successfully written master Oscilar landing page!');
